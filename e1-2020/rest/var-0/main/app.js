...
app.post('/ships', async (req, res) => {
    if(Object.keys(req.body).length==0) res.status(400).json({message:"body is missing"});
      else if(req.body.displacement<1000)
    {
        res.status(400).json({message:"displacement should be over 1000"});
    }
    else if(req.body.portOfSail===undefined || req.body.name ===undefined || req.body.displacement ===undefined)
